...
//        }
    ...
    public List<String> findHous(QueryFilter queryFilter) {
        ...
        List<RangeDTO> maps = gridBasicInfoService.getGridsHouseBymassifId((String) querys.get(0).getValue());
        return maps.stream().map(e -> (String) e.getId()).collect(Collectors.toList());
    }
}
